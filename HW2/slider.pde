class Slider extends Button{
    ...
    public void click(){
        ...
        if(!checkInSide() &&  !press) return;
        if(mousePressed){
            press = true;
            if(vertical)pos = new Vector3(max( start_end_point.x,min(start_end_point.y,mouseX)), pos.y,0);
            else pos = new Vector3(pos.x,max( start_end_point.x,min(start_end_point.y,mouseY)),0);
        }else{
          press = false;
          once = false;
        }
