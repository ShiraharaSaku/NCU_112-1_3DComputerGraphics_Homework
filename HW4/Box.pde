public class Box {
    ...
    public boolean checkInSide() {
        if (mouseX>=pos.x && mouseX<=pos.x+size.x && mouseY>=pos.y && mouseY<=pos.y+size.y) return true;
        return false;
    }
}
...
public class Button extends Box {
    ...
    @Override
        public void show() {
        if (!press)fill(box_color);
        else fill(click_color);
        noStroke();
        rect(pos.x, pos.y, size.x, size.y);
        if (image!=null) image(image, pos.x, pos.y, size.x, size.y);
    }
