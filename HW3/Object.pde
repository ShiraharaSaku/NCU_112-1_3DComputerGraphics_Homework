public class GameObject {
    ...
    void debugDraw() {
        ...
        for (int i=0; i<mesh.triangles.size(); i++) {
            ...
            if (img_pos.length >= 3){
                Vector3 n = Vector3.cross(img_pos[2].sub(img_pos[0]), img_pos[1].sub(img_pos[0]));
                ...
                if (n.z < 0) {
                    ...
                    */
                    for(int j=0;j<img_pos.length;j++){          
                        CGLine(img_pos[j].x,img_pos[j].y,img_pos[(j+1)%img_pos.length].x,img_pos[(j+1)%img_pos.length].y);
                    }
                }
            }
    ...
    Matrix4 localToWorld() {
        ...
        //m = m.mult(Matrix4.Trans(transform.position));
        
        
        m = m.mult(Matrix4.Trans(transform.position.mult(-1))); 
        m = m.mult(Matrix4.RotY(transform.rotation.y));
        m = m.mult(Matrix4.RotX(transform.rotation.x));
        m = m.mult(Matrix4.RotZ(transform.rotation.z));
        m = m.mult(Matrix4.Scale(transform.scale));
